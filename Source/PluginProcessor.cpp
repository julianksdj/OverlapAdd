...
void FftPassthroughAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    juce::dsp::WindowingFunction<float> hamming(FFT_SIZE, juce::dsp::WindowingFunction<float>::WindowingMethod::hamming, true, 0);
    hammingArray = new float[FFT_SIZE];
    hamming.fillWindowingTables(hammingArray, FFT_SIZE, juce::dsp::WindowingFunction<float>::WindowingMethod::hamming);
}
...
void FftPassthroughAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int i=0; i<currentBufferSize; i++) {
        ...
        inBuffer[inWritePointer++] = channelDataLeft[i];
        ...
        out *= (float) HOP_SIZE / (float) FFT_SIZE; // out * 0.0625
        outReadPointer++;
...
void FftPassthroughAudioProcessor::processFft() {
    ...
    for (int i=0; i<FFT_SIZE; i++) {
        int readIdx = (inWritePointer + i - FFT_SIZE + CBUFFER_SIZE) % CBUFFER_SIZE;
        inFft[i] = inBuffer[readIdx];
    }
    ...
    for (int i=0; i<FFT_SIZE; i++) {
        int writeIdx = (outWritePointer + i) % CBUFFER_SIZE;
        outBuffer[writeIdx] += (outIfft[i] / FFT_SIZE);
    }
